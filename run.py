    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT: sys.exit()
            ...
            if selected_answer is not None and not question_answered:
                # Obtenir la réponse correspondant au chiffre sélectionné depuis la question
                selected_option = question.options[selected_answer - 1]

                # Vérifie si la réponse sélectionnée correspond à la réponse correcte
                if selected_option == question.answer:
                    # Si la réponse est correcte
                    print("Bonne réponse !")
                    player.score_point(question.theme)  # Marque un point pour la question
                    question_answered = True
                    
                    # Affiche "Tu as gagné ! Relance le dé" après avoir gagné
                    winning_text = font.render("Tu as gagné ! Relance le dé", True, WHITE)
                    screen.blit(winning_text, (question_zone.x, question_zone.y))  # Affiche le message de victoire
                    pygame.display.flip()  # Met à jour l'écran pour afficher le message
                    last_question = question
                    
                    # Masque la question après avoir affiché le message de victoire
            ...
            # When player click on the case choosen
                ...
                for idx, pos in enumerate(positions):
                    # Vérifie si le clic est à l'intérieur du cercle entouré en rouge
                    if pygame.mouse.get_pos()[0] >= pos[0] - BOX_RADIUS - 3 and pygame.mouse.get_pos()[0] <= pos[0] + BOX_RADIUS + 3:
                        if pygame.mouse.get_pos()[1] >= pos[1] - BOX_RADIUS - 3 and pygame.mouse.get_pos()[1] <= pos[1] + BOX_RADIUS + 3:
                            ...
                            # Efface tout le plateau
                            screen.fill(BACKGROUND)

                            # Redessine chaque case avec sa couleur d'origine
                            for idx, pos in enumerate(positions):
                                box_color = BOX_TYPE_TO_COLOR[network.nodes[idx]['box_type']]
                                pygame.draw.circle(screen, box_color, pos, BOX_RADIUS)

                            # Dessine le cercle du joueur dans sa nouvelle position
